...
from pyspark.sql.types import StructType, StructField, StringType, FloatType # type: ignore
...
def create_dataframe(spark_session):
    ...
    try:
        df = spark_session \
            .readStream \
            .format("kafka") \
            .option("kafka.bootstrap.servers", config['upstash_info']['bootstrap_server']) \
            .option("subscribe", topic) \
            .option("kafka.sasl.mechanism", "SCRAM-SHA-256") \
            .option("kafka.security.protocol", "SASL_SSL") \
            .option("kafka.sasl.jaas.config", 
                    f"org.apache.kafka.common.security.scram.ScramLoginModule required " +
                    f"username='{config['upstash_info']['sasl_plain_username']}' " +
                    f"password='{config['upstash_info']['sasl_plain_password']}';") \
            .load()
